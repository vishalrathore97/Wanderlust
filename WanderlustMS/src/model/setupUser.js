...
exports.userSetup = () => {
    return conn.getUserCollection().then( ( userCollection ) => {
        return userCollection.deleteMany().then( () => {
            return userCollection.insertMany( userData ).then( () => {
                return conn.getBookingsCollection().then( bookingsCollection=>{
                    return bookingsCollection.deleteMany().then( ()=>{
                        return bookingsCollection.insertMany( bookingData ).then( ()=>{
                            return conn.getDestinationsCollection().then( destCollection=>{
                                return destCollection.deleteMany().then( ()=>{
                                    return destCollection.insertMany( destinations ).then( ()=>{
                                        return conn.getHotdealsCollection().then( hotDealsCollection=>{
                                            return hotDealsCollection.deleteMany().then( ()=>{
                                                return hotDealsCollection.insertMany( hotDeals ).then( result=>{
                                                    if( result ) return"Insertion Successful";
